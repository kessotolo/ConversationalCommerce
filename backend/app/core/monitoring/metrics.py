...
disk_total = Gauge('system_disk_total_bytes', 'Total disk space in bytes', ['mount_point'])
process_count = Gauge('system_process_count', 'Number of running processes')
open_file_descriptors = Gauge('system_open_file_descriptors', 'Number of open file descriptors')
...
active_users = Gauge('app_active_users', 'Number of active users', ['tenant_id'])
active_sessions = Gauge('app_active_sessions', 'Number of active sessions')
failed_logins = Counter('app_failed_logins_total', 'Number of failed login attempts', ['tenant_id'])
successful_logins = Counter('app_successful_logins_total', 'Number of successful logins', ['tenant_id'])

# Database metrics
db_connection_pool_size = Gauge('db_connection_pool_size', 'Database connection pool size')
db_connection_pool_used = Gauge('db_connection_pool_used', 'Database connections in use')
...
# Business metrics
order_total = Counter('business_order_total', 'Total orders', ['tenant_id', 'status'])
payment_total = Counter('business_payment_total', 'Total payments', ['tenant_id', 'provider', 'status'])
user_registration = Counter('business_user_registration_total', 'User registrations', ['tenant_id'])
conversation_started = Counter('business_conversation_started_total', 'Conversations started', ['tenant_id'])
conversation_completed = Counter('business_conversation_completed_total', 'Conversations completed', ['tenant_id'])
conversation_abandoned = Counter('business_conversation_abandoned_total', 'Conversations abandoned', ['tenant_id'])

# Resource utilization tracking
api_rate_limit_hit = Counter('api_rate_limit_hit_total', 'API rate limit hits', ['endpoint'])
error_rate = Counter('error_rate_total', 'Error count', ['error_type', 'endpoint'])
...
class MetricsCollector:
    ...
    def __init__(self):
        ...
        self.push_gateway_url = getattr(settings, 'PROMETHEUS_PUSHGATEWAY_URL', None)
        self.enable_metrics_endpoint = getattr(settings, 'ENABLE_METRICS_ENDPOINT', False)
    ...
    def start(self):
        ...
        if self.enable_metrics_endpoint:
            try:
                ...
                logger.info(f"Metrics HTTP server started on port {self.metrics_port}")
    ...
    def track_payment(self, tenant_id: str, provider: str, status: str):
        """Track payment"""
        payment_total.labels(tenant_id=tenant_id, provider=provider, status=status).inc()
