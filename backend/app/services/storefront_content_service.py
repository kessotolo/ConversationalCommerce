...
async def get_storefront_metadata(db: Session, tenant_id: uuid.UUID) -> Dict[str, Any]:
    ...
    return {
        ...
        or f"Welcome to {tenant.name}'s online store. Shop our products and enjoy great service.",
...
async def get_featured_products(
    db: Session, tenant_id: uuid.UUID, limit: int = 8, skip: int = 0
) -> List[ProductModel]:
    """
...
async def get_product_categories(
    ...
    categories = (
        db.query(
            ProductModel.category, func.count(ProductModel.id).label("product_count")
        )
        .filter(
            ProductModel.tenant_id == tenant_id,
            not ProductModel.is_deleted,
...
async def get_products_by_category(
    db: Session, tenant_id: uuid.UUID, category: str, limit: int = 20, skip: int = 0
    ...
    products = (
        base_query.order_by(
            desc(ProductModel.is_featured), desc(ProductModel.created_at)
        )
        .offset(skip)
        .limit(limit)
        .all()
...
async def get_product_detail(
    ...
    related_products = related_products_query.order_by(func.random()).limit(4).all()

    return {"product": product, "related_products": related_products}


async def search_products(
    db: Session, tenant_id: uuid.UUID, query: str, limit: int = 20, skip: int = 0
) -> Tuple[List[ProductModel], int]:
    """
    ...
    navigation = {
        ...
        "footer_menu": {
            ...
            "social": [
                {
                    "label": "WhatsApp",
                    "url": config.social_links.get("whatsapp", ""),
                    "type": "social",
                },
                {
                    "label": "Instagram",
                    "url": config.social_links.get("instagram", ""),
                    "type": "social",
                },
                {
                    "label": "Facebook",
                    "url": config.social_links.get("facebook", ""),
                    "type": "social",
                },
                {
                    "label": "Twitter",
                    "url": config.social_links.get("twitter", ""),
                    "type": "social",
                },
                {
                    "label": "TikTok",
                    "url": config.social_links.get("tiktok", ""),
                    "type": "social",
                },
            ],
        },
