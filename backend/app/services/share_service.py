...
class ShareService:
    def _get_product_and_verify_tenant(
        ...
        if not tenant.is_verified:
            raise ValueError("Tenant is not verified for sharing features")
    ...
    def _get_utm_url(
        ...
        base_url = product.storefront_url or f"https://yourstore.com/products/{product.id}"
    ...
    def generate_whatsapp_share_link(
        ...
        product, tenant = self._get_product_and_verify_tenant(db, product_id, tenant_id)
        
        # Generate UTM URL
        utm_url, utm_params = self._get_utm_url(product, campaign, source, medium)

        # Generate WhatsApp share link
        caption = f"Check out this product: {product.name}! {utm_url}"
        whatsapp_link = generate_whatsapp_link(
            tenant.whatsapp_number or "",
            product.name,
            caption
        )

        # Log the share event
        self._log_share_event(db, user_id, product_id, "whatsapp", campaign, utm_params)
    ...
    def generate_instagram_share(
        ...
        # Log the share event
        self._log_share_event(db, user_id, product_id, "instagram", campaign, utm_params)
    ...
    def generate_facebook_share_link(
        ...
        # Log the share event
        self._log_share_event(db, user_id, product_id, "facebook", campaign, utm_params)
    ...
    def generate_tiktok_share(
        ...
        # Log the share event
        self._log_share_event(db, user_id, product_id, "tiktok", campaign, utm_params)
    ...
    def generate_twitter_share_link(
        ...
        # Log the share event
        self._log_share_event(db, user_id, product_id, "twitter", campaign, utm_params)
    ...
    def generate_telegram_share_link(
        ...
        telegram_link = generate_telegram_share_link(
            url=utm_url,
            text=text
        )

        # Log the share event
        self._log_share_event(db, user_id, product_id, "telegram", campaign, utm_params)
    ...
    def generate_share_links(
        ...
        for platform in platforms:
            try:
                ...
                logger.error(f"Failed to generate {platform} share link: {str(e)}")
