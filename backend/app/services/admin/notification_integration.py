...
class AdminNotificationService:
    ...
    async def notify_all_admins(
        ...
        try:
            ...
            async with get_db() as db:
                admin_roles = [role for role in AdminRole if role != AdminRole.CUSTOM]
                admins_by_role = {}
            ...
            logger.info(f"Notification sent to {len(admin_ids)} admins: {notification_id}")
    ...
    async def notify_admins_by_role(
        ...
        Returns:
            Notification ID
        """
        logger.info(f"Sending notification to admins with roles {roles}: {title}")
    ...
    async def notify_tenant_admins(
        ...
        Returns:
            Notification ID
        """
        logger.info(f"Sending notification to tenant {tenant_id} administrators: {title}")
    ...
    def send_system_alert(
        ...
        Returns:
            Notification ID
        """
        logger.info(f"Sending system alert ({severity}): {alert_type} - {affected_component}")
        ...
        target_roles = [AdminRole.SUPER_ADMIN]  # Super admins always get alerts
        ...
        if alert_type.lower() == "security":
            ...
            target_roles.extend([AdminRole.SYSTEM_ADMIN, AdminRole.SUPPORT_ADMIN])
    ...
    def send_task_notification(
        ...
        Returns:
            Notification ID
        """
        logger.info(f"Sending task notification: {task_name} ({task_id}) - {status}")
        ...
        message = message_map.get(status.lower(), f"Task '{task_name}' status: {status}")
