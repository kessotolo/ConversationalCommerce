...
async def get_product_collections(
    ...
    collections = (
        db.query(
            ProductModel.collection, func.count(ProductModel.id).label("product_count")
        )
        .filter(
            ProductModel.tenant_id == tenant_id,
            not ProductModel.is_deleted,
        ...
        .group_by(ProductModel.collection)
...
async def get_product_tags(
    db: Session, tenant_id: uuid.UUID, limit: int = 20
) -> List[Dict[str, Any]]:
    """
    ...
    result = db.execute(query, {"tenant_id": str(tenant_id), "limit": limit}).fetchall()
...
async def get_product_detail_for_storefront(
    db: Session, tenant_id: uuid.UUID, product_id: uuid.UUID
) -> Dict[str, Any]:
    """
    ...
    related_products = (
        ...
        .order_by(desc(ProductModel.is_featured), func.random())
        .limit(4)
        .all()
    )
    ...
    recommended_products = (
        recommended_query.order_by(desc(ProductModel.is_featured), func.random())
        .limit(4)
        .all()
    )
        ...
        variants = (
            ...
            .filter(
                ...
                or_(
                    ...
                    and_(ProductModel.id == base_id, ProductModel.id != product_id),
                ),
            )
            .all()
        )
...
async def transform_product_images(
    product: Dict[str, Any], sizes: List[str] = None
) -> Dict[str, Any]:
    """
    ...
    if "images" in result and result["images"]:
        ...
        for image_url in result["images"]:
            image_variations = {"original": image_url}
                ...
                if size in size_dimensions:
                    ...
                    if "cloudinary.com" in image_url:
                        ...
                        if len(parts) == 2:
                            ...
                            transform = (
                                f"c_fit,w_{dim.split('x')[0]},h_{dim.split('x')[1]}/"
                            )

                            image_variations[size] = f"{base}{transform}{path}"
                    else:
                        # For other image services, could add query parameters
                        image_variations[size] = (
                            f"{image_url}?size={size_dimensions[size]}"
                        )
...
async def get_new_arrivals(
    db: Session, tenant_id: uuid.UUID, limit: int = 8
) -> List[ProductModel]:
    """
...
async def get_bestsellers(
    ...
    products = (
        ...
        .order_by(
            desc(ProductModel.popularity_score)
            if hasattr(ProductModel, "popularity_score")
            else desc(ProductModel.created_at)
        )
        .limit(limit)
        .all()
    )
