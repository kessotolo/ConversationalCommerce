...
from backend.app.models.storefront_asset import AssetType, StorefrontAsset
...
UPLOAD_BASE_DIR = getattr(settings, "ASSET_UPLOAD_DIR",
                          "/tmp/storefront_assets")
MAX_UPLOAD_SIZE_MB = int(getattr(settings, "MAX_UPLOAD_SIZE_MB", 10))
...
async def upload_asset(
    ...
    if asset_type == AssetType.IMAGE:
        # Placeholder for future asset moderation logic.
        pass
...
async def update_asset(
    ...
    Raises:
        ...
        HTTPException: 403 if user doesn't have permission
        HTTPException: 409 if asset was modified by another process
    """
    # Check if tenant exists
    tenant = db.query(Tenant).filter(Tenant.id == tenant_id).first()
    if not tenant:
    ...
    if metadata is not None:
        ...
        asset.asset_metadata = {**asset.asset_metadata, **metadata}
...
async def get_asset_url(
    ...
    if asset.is_optimized and width and height and asset.asset_type == AssetType.IMAGE:
        optimized_versions = asset.asset_metadata.get("optimized_versions", {})
