        ...
        try:
            ...
            if data["status"] and data["data"]["status"] == "success":
                ...
                return PaymentVerificationResponse(
                    ...
                    payment_method=self._map_payment_method(data["data"].get("channel")),
                    fees=TransactionFee(
...
class FlutterwaveProvider(PaymentProviderInterface):
    ...
    def __init__(self, secret_key: str, public_key: str, encryption_key: Optional[str] = None):
        self.secret_key = secret_key
        self.public_key = public_key
    ...
    def verify_payment(self, reference: str) -> PaymentVerificationResponse:
        ...
        try:
            ...
            response = requests.get(
                f"{self.base_url}/transactions?tx_ref={reference}",
                headers=headers
            )
                ...
                if transaction["status"] == "successful":
                    ...
                    return PaymentVerificationResponse(
                        ...
                        customer={
                            ...
                            "phone": transaction.get("customer", {}).get("phone_number", "")
                        },
                        payment_method=self._map_payment_method(transaction.get("payment_type")),
                        fees=TransactionFee(
                    ...
                    logger.error(f"Flutterwave transaction not successful: {transaction}")
                    return PaymentVerificationResponse(
                        success=False,
                        reference=reference,
                        amount=Money(value=float(transaction.get("amount", 0)), currency=transaction.get("currency", "NGN")),
                        provider=PaymentProvider.FLUTTERWAVE,
                        transaction_date=transaction.get("created_at", "")
                    )
            else:
    ...
    def validate_webhook(self, payload: bytes, signature: str) -> bool:
        ...
        try:
            ...
            data_to_hash = f"{self.secret_key}"  # In practice, concatenate with relevant fields
