...
class TenantService:
    ...
    async def create_tenant(
        ...
        **kwargs
        ...
        if existing_subdomain:
            raise DuplicateEntityException(f"Subdomain '{subdomain}' is already taken")
        
        # Check if custom domain already exists (if provided)
        if custom_domain:
            existing_domain = await self.get_tenant_by_domain(db, domain=custom_domain)
            if existing_domain:
                raise DuplicateEntityException(f"Domain '{custom_domain}' is already registered")
    ...
    async def get_tenant_by_id(
        ...
        *,
        ...
        if not tenant:
            raise EntityNotFoundException(f"Tenant with ID {tenant_id} not found")
        return tenant
        ...
        *,
        ...
        if filter_params:
            if "name" in filter_params:
                query = query.where(Tenant.name.ilike(f"%{filter_params['name']}%"))
            if "is_active" in filter_params:
                query = query.where(Tenant.is_active == filter_params["is_active"])
