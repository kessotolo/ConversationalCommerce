...
class TwoFactorService:
    ...
    def __init__(self):
        ...
        self.fernet = self._create_fernet_from_secret(settings.SECRET_KEY, "totp_secret_encryption")
    ...
    def _generate_backup_codes(self, count: int = 10) -> Tuple[List[str], List[str]]:
        ...
        backup_codes = [
            ''.join(secrets.choice('23456789ABCDEFGHJKLMNPQRSTUVWXYZ') for _ in range(10))
    ...
    async def verify_totp(
        ...
        # Decrypt the secret
        try:
            secret = self.fernet.decrypt(totp_record.encrypted_secret.encode()).decode()
        except Exception:
        ...
        # Verify the code
        is_valid = totp.verify(code, valid_window=1)  # Allow 1 period before/after for clock skew
    ...
    async def is_totp_required(self, db: AsyncSession, admin_user: AdminUser) -> bool:
        ...
        query = select(AdminTOTPRequirement).where(
            or_(
                ...
                and_(
                    AdminTOTPRequirement.role_id.in_([r.id for r in admin_user.roles]),
