...
async def assign_role(
    ...
    permission = db.query(StorefrontPermission).filter(
...
async def set_section_permission(
    ...
    section_permissions = permission.section_permissions.copy() if permission.section_permissions else {}
...
async def set_component_permission(
    ...
    component_permissions = permission.component_permissions.copy() if permission.component_permissions else {}
...
async def list_users_with_permissions(
    ...
    permissions = db.query(StorefrontPermission).filter(
        StorefrontPermission.tenant_id == tenant_id
    ).all()
...
async def remove_user_permission(
    ...
    await log_permission_change(
        ...
        details={
            "previous_role": permission.role.value
        }
    )
...
async def validate_json_schema(data: Dict[str, Any], schema: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
    """
...
async def validate_theme_settings(theme_settings: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
    """
...
async def validate_layout_config(layout_config: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
    """
    ...
    schema = {
        ...
        "properties": {
            ...
            "categories": {
                ...
                "properties": {
                    "enabled": {"type": "boolean"},
                    "display_mode": {"type": "string", "enum": ["grid", "list", "carousel"]}
                },
                "required": ["enabled"]
            },
            "about": {
...
async def sanitize_html_content(content: str) -> str:
    ...
    try:
        ...
        allowed_tags = [
            ...
            'nl', 'ol', 'p', 'pre', 'span', 'strong', 'table', 'tbody',
            'td', 'th', 'thead', 'tr', 'ul'
        ]
