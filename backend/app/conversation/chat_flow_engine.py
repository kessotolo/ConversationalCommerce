...
class ChatFlowEngine:
    ...
    def handle_input(self, message: str) -> List[Dict[str, Any]]:
        ...
        if self._is_cancel_intent(message):
            logging.info(f"[ChatFlow] Cancel intent detected. Resetting state.")
        ...
        if step == ChatStep.ASK_NAME:
            ...
            if not self._validate_address(message):
                logging.warning(f"[ChatFlow] Invalid address input: '{message}'")
                return self._retry_or_fail(
            ...
            self.state["retries"] = 0
            logging.info(f"[ChatFlow] Address accepted. Moving to ASK_PAYMENT.")
            ...
            self.state["retries"] = 0
            logging.info(f"[ChatFlow] Payment method accepted. Moving to CONFIRM.")
            ...
            if message.strip().lower() == "confirm":
                try:
                    ...
                    logging.error(f"[ChatFlow] Error during order/payment: {str(e)}")
                    messages.append(
                        self.message_builder.text_message(
                            "Sorry, there was an error placing your order. Please try again later."
                        )
                ...
                logging.warning(f"[ChatFlow] Invalid confirm input: '{message}'")
                return self._retry_or_fail(
                    ChatStep.CONFIRM,
                    "Please reply 'confirm' to place your order, or 'edit' to change details.",
                )
        elif step == ChatStep.COMPLETE:
            logging.info(f"[ChatFlow] Order complete for phone {self.phone_number}.")
            messages.append(
                self.message_builder.text_message("Your order is complete. Thank you!")
            )
        self._save_state()
        return messages
    ...
    def _prompt_for_step(self, step: ChatStep) -> Dict[str, Any]:
        ...
        return prompts.get(
            step, self.message_builder.text_message("Let's continue your order.")
        )
