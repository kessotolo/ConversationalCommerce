...
class IPAllowlistEntry(Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint('ip_range', 'user_id', 'role_id', 'tenant_id', 'is_global', name='uq_ip_allowlist_scope'),
...
class IPAllowlistSetting(Base):
    ...
    temporary_bypass_duration_minutes = Column(Integer, nullable=False, default=60)
    
    # Geolocation settings (optional)
    geo_restrict_countries = Column(ARRAY(String(2)), nullable=True)  # ISO country codes to restrict to
    geo_block_countries = Column(ARRAY(String(2)), nullable=True)     # ISO country codes to block
    ...
    __table_args__ = (
        ...
        UniqueConstraint('tenant_id', 'role_id', name='uq_ip_allowlist_setting_scope'),
        # Indexes
...
class IPTemporaryBypass(Base):
    ...
    # Verification method
    verification_method = Column(String(50), nullable=False)  # email, sms, etc.
