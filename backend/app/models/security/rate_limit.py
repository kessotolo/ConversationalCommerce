...
class LoginAttempt(Base):
    ...
    username = Column(String(255), nullable=False)  # Store the username even if user doesn't exist
...
class AccountLockout(Base):
    ...
    # Lockout details
    reason = Column(String(50), nullable=False)  # e.g., "failed_attempts", "suspicious_activity"
    locked_at = Column(DateTime(timezone=True), nullable=False, default=lambda: datetime.now(timezone.utc))
    expires_at = Column(DateTime(timezone=True), nullable=True)  # Null for manual unlock required
    is_active = Column(Boolean, nullable=False, default=True)
    
    # Who locked and unlocked the account
    locked_by_system = Column(Boolean, nullable=False, default=True)  # True if automatic
    locked_by_user_id = Column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=True)  # Admin who locked
    unlocked_by_user_id = Column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=True)  # Admin who unlocked
    unlocked_at = Column(DateTime(timezone=True), nullable=True)
    
    # Additional context
    triggering_ip = Column(String(45), nullable=True)
    failed_attempts = Column(Integer, nullable=True)  # Number of failed attempts that triggered lockout
    details = Column(JSONB, nullable=True)  # Additional context about the lockout
...
class RateLimitRule(Base):
    ...
    endpoint = Column(String(255), nullable=True)  # API endpoint pattern or null for global
...
class RateLimitEntry(Base):
    ...
    rule_id = Column(UUID(as_uuid=True), ForeignKey("rate_limit_rules.id"), nullable=False)
    ...
    # Timeframe
    window_start = Column(DateTime(timezone=True), nullable=False, default=lambda: datetime.now(timezone.utc))
    ...
    __table_args__ = (
        ...
        UniqueConstraint('ip_address', 'user_id', 'rule_id', name='uq_rate_limit_entries_ip_user_rule'),
        # Indexes
