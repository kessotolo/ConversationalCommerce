...
class AnalyticsMetric(Base):
    ...
    time_period = Column(String(20), index=True, nullable=False)  # daily, weekly, monthly, yearly
    time_value = Column(String(20), index=True, nullable=False)  # ISO format date or period identifier
    start_date = Column(DateTime(timezone=True), nullable=False)
    end_date = Column(DateTime(timezone=True), nullable=False)
    is_final = Column(Boolean, default=True, nullable=False)  # Indicates if metric is subject to change
    created_at = Column(DateTime(timezone=True), server_default=func.now(), nullable=False)
...
class AnalyticsReport(Base):
    ...
    date_range = Column(JSON, nullable=False)  # start_date, end_date, or preset like "last_30_days"
    visualization_type = Column(String(50), nullable=False)
    visualization_config = Column(JSON, nullable=True)
    is_scheduled = Column(Boolean, default=False, nullable=False)
    schedule_config = Column(JSON, nullable=True)  # frequency, recipients, etc.
...
Tenant.analytics_events = relationship("AnalyticsEvent", back_populates="tenant")
Tenant.analytics_metrics = relationship("AnalyticsMetric", back_populates="tenant")
Tenant.analytics_reports = relationship("AnalyticsReport", back_populates="tenant")
