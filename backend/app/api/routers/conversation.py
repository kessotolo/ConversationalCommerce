...
def get_conversation_events(db: Session = Depends(get_db)):
    events = (
        db.query(ConversationEvent).order_by(ConversationEvent.created_at.desc()).all()
    )
    return events
...
def log_conversation_event(
    ...
    try:
        ...
        key_types = {"message_sent", "message_read", "product_clicked", "order_placed"}
...
def get_conversation_analytics(
    ...
    try:
        ...
        # Count by event type
        counts_by_type = (
            db.query(ConversationEvent.event_type, func.count(ConversationEvent.id))
            .filter(
    ...
    try:
        ...
        for (conv_id,) in conv_ids:
            ...
            for sent in sent_times:
                ...
                if after_reads:
                    response_times.append((after_reads[0] - sent).total_seconds())
            avg_response_time = (
...
def export_conversation_analytics_csv(db: Session = Depends(get_db)):
    ...
    try:
        ...
        writer.writerow(
            ["id", "conversation_id", "user_id", "event_type", "created_at", "payload"]
        )
        for e in events:
            writer.writerow(
                [
