...
def facebook_share_link(
    ...
    medium: str = Query("share", description="UTM medium"),
    hashtag: Optional[str] = Query(None, description="Optional hashtag for the Facebook share"),
    db: Session = Depends(deps.get_db),
    user: ClerkTokenData = Depends(deps.get_current_user)
):
    """
...
def twitter_share_link(
    ...
    medium: str = Query("share", description="UTM medium"),
    hashtags: Optional[List[str]] = Query(None, description="Optional hashtags for the Twitter share"),
    via: Optional[str] = Query(None, description="Optional Twitter handle to attribute the share to"),
    db: Session = Depends(deps.get_db),
    user: ClerkTokenData = Depends(deps.get_current_user)
):
    """
...
def generate_qr_code(
    ...
    logo: bool = Query(False, description="Include tenant logo in QR code if available"),
    db: Session = Depends(deps.get_db),
    user: ClerkTokenData = Depends(deps.get_current_user)
):
    """
    ...
    try:
        ...
        if logo and hasattr(tenant, 'logo_url') and tenant.logo_url:
            try:
                ...
                img.paste(logo_img, position, logo_img if logo_img.mode == 'RGBA' else None)
