...
async def resolve_violation(
    *,
    ...
    resolved = violation_service.resolve_violation(
        ...
        resolution_notes=update_in.notes
    )
    return resolved
...
async def violation_stats(
    *,
    ...
    total = db.query(func.count(Violation.id)).filter(
        Violation.tenant_id == tenant_id).scalar()
    by_type = dict(db.query(Violation.type, func.count(Violation.id)).filter(
        Violation.tenant_id == tenant_id).group_by(Violation.type).all())
    by_severity = dict(db.query(Violation.severity, func.count(Violation.id)).filter(
        Violation.tenant_id == tenant_id).group_by(Violation.severity).all())
    by_action = dict(db.query(Violation.action, func.count(Violation.id)).filter(
        Violation.tenant_id == tenant_id).group_by(Violation.action).all())
    by_status = dict(db.query(Violation.status, func.count(Violation.id)).filter(
        Violation.tenant_id == tenant_id).group_by(Violation.status).all())
    return {
        "total": total,
        "by_type": by_type,
