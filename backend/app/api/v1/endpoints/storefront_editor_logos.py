...
from app.models.storefront_logo import LogoType, LogoStatus
from app.schemas.storefront_logo import LogoCreate, LogoResponse, LogoList, LogoUpdate
from app.services import storefront_logo_service

router = APIRouter()


@router.post("/{tenant_id}/logos", response_model=LogoResponse, status_code=status.HTTP_201_CREATED)
...
async def delete_logo(
    ...
    try:
        ...
        return {
            "success": deleted,
            "message": "Logo deleted successfully" if deleted else "Logo not found or already deleted"
        }
    except HTTPException as e:
        raise e
    except Exception as e:
...
@router.get("/{tenant_id}/logos/active/{logo_type}", response_model=Optional[LogoResponse])
