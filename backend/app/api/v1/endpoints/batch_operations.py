...
class ProductBatchUpdatePayload(BaseModel):
    ...
    products: List[ProductBatchUpdateItem] = Field(..., min_length=1, max_length=100, 
                                               description="List of products to update")
    update_data: Dict[str, Any] = Field(..., description="Fields to update for all products")
...
async def batch_update_products_endpoint(
    ...
    current_user: ClerkTokenData = Depends(require_seller)  # Require seller role
):
    """
