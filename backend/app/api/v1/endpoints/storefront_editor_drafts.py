...
from app.schemas.storefront_draft import (
...
@router.post(
    "/{tenant_id}/drafts",
    response_model=StorefrontDraftResponse,
...
async def list_storefront_drafts(
    ...
    try:
        drafts, total = await storefront_service.list_drafts(
            db=db, tenant_id=tenant_id, user_id=user_id, skip=skip, limit=limit
        )

        return {"items": drafts, "total": total, "skip": skip, "limit": limit}
    except HTTPException as e:
        raise e
    except Exception as e:
        raise HTTPException(
...
async def get_storefront_draft(
    ...
    try:
        draft = await storefront_service.get_draft(
            db=db, tenant_id=tenant_id, draft_id=draft_id, user_id=user_id
        )
    ...
    try:
        ...
        return {
            ...
            "message": (
                ...
                else "Draft scheduled for publishing"
            ),
            "version_id": (
                str(result.get("version_id")) if result.get("version_id") else None
            ),
