...
from app.app.schemas.storefront_permission import (
...
from app.app.services import storefront_permissions_service
...
async def list_user_permissions(
    ...
    try:
        ...
        if not has_perm:
            ...
            return {"items": [user_perms], "total": 1}

        # If user has permissions, return all user permissions
        users_with_permissions = (
            await storefront_permissions_service.list_users_with_permissions(
                db=db, tenant_id=tenant_id
            )
        )

        return {"items": users_with_permissions, "total": len(users_with_permissions)}
...
@router.put(
    "/{tenant_id}/permissions/{user_id}/role", response_model=PermissionResponse
)
...
@router.put(
    "/{tenant_id}/permissions/{user_id}/section", response_model=PermissionResponse
)
...
@router.put(
    "/{tenant_id}/permissions/{user_id}/component", response_model=PermissionResponse
)
...
async def remove_user_permission(
    ...
    try:
        removed = await storefront_permissions_service.remove_user_permission(
            db=db, tenant_id=tenant_id, user_id=user_id, removed_by=current_user_id
        )
        ...
        return {
            ...
            "message": (
                ...
                if removed
                else "No permissions found to remove"
            ),
        }
    except HTTPException as e:
        raise e
...
async def get_audit_log(
    ...
    try:
        ...
        # FUTURE: Implement actual audit log retrieval once it's available. See issue #131 for tracking.
