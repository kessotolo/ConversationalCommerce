...
class EventBus:
    ...
    async def publish(self, event: DomainEvent) -> None:
        ...
        # Log the event
        logger.info(f"Event published: {event_type} - {event.event_id} - tenant: {event.tenant_id}")
    ...
    async def _execute_handler(self, handler: EventHandler, event: DomainEvent) -> None:
        ...
        try:
            ...
            logger.error(f"Error in event handler for {event.event_type}: {str(e)}", exc_info=True)
    ...
    def get_recent_events(self, limit: int = 100, event_type: Optional[str] = None) -> List[DomainEvent]:
        """
