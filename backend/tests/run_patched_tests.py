...
os.environ['TRANSFORMERS_CACHE'] = '/tmp/nonexistent_path'  # Point to non-existent cache
...
try:
    ...
    mock_huggingface_hub.hf_hub_download = MagicMock(side_effect=RuntimeError("Mocked hf_hub_download"))
    mock_huggingface_hub.snapshot_download = MagicMock(side_effect=RuntimeError("Mocked snapshot_download"))
...
logger.info(f"Running: {sys.executable} -m pytest --log-cli-level=DEBUG {' '.join(pytest_args)}")
if len(sys.argv) > 1:
    pytest_args.extend(sys.argv[1:])
else:
    # Default: Run a single test to avoid hanging
    pytest_args.append("tests/api/test_create_order.py::test_create_order_minimum_fields")

result = subprocess.run([sys.executable, "-m", "pytest", "--log-cli-level=DEBUG"] + pytest_args)
