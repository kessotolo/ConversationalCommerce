...
def test_mpesa_initialize_payment():
    with patch("app.services.payment.payment_provider.MpesaProvider._get_access_token", return_value="mock_token"), \
...
def test_payment_status_mapping():
    ...
    assert service.map_provider_status_to_internal(
        "paystack", "success") == "COMPLETED"
    assert service.map_provider_status_to_internal(
        "flutterwave", "successful") == "COMPLETED"
    assert service.map_provider_status_to_internal(
        "mpesa", "Success") == "COMPLETED"
    assert service.map_provider_status_to_internal(
        "paystack", "failed") == "FAILED"
    assert service.map_provider_status_to_internal(
        "mpesa", "Pending") == "PENDING"
