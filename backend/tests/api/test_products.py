...
def setup_test_user(db_session: Session, test_tenant):
    ...
    # Create test user
    test_user = User(email="testuser@example.com", is_seller=True)
...
def test_products_require_auth():
    ...
    for route in router.routes:
        ...
        for param_name, param in signature.parameters.items():
            ...
            if param.default != inspect.Parameter.empty:
                ...
                elif hasattr(param.default, "dependency") and callable(
    ...
    assert (
        found_auth_dependency
    ), "No authentication dependencies found in product router endpoints"
