...
# Using a revision ID from the list that appears recent
down_revision = 'f3c8e0fb295b'
branch_labels = None
depends_on = None
...
def upgrade():
    # Create enum types
    create_enum('storefrontstatus', [
                'draft', 'published', 'scheduled', 'archived'])
    create_enum('storefrontrole', ['viewer', 'editor', 'publisher', 'admin'])
    create_enum('storefrontsectiontype', [
                'theme', 'layout', 'content', 'products', 'settings', 'banners', 'assets', 'seo'])
    create_enum('assettype', ['image', 'video', 'document', 'audio', 'other'])
    create_enum('bannertype', ['hero', 'promotional',
                'announcement', 'category', 'seasonal', 'custom'])
    create_enum('bannerplacement', [
                'header', 'homepage', 'category_page', 'product_page', 'footer', 'sidebar', 'popup'])
    create_enum('logovariant', ['primary', 'secondary',
                'dark', 'light', 'monochrome', 'favicon', 'mobile'])
    ...
    create_enum('componentplacement', [
                'header', 'main', 'sidebar', 'footer', 'popup', 'product_page', 'category_page', 'checkout'])
    create_enum('pagetype', ['home', 'product', 'category', 'about',
                'contact', 'custom', 'checkout', 'cart', 'account', 'policy'])
    ...
    op.add_column('storefront_configs', sa.Column(
        'published_at', sa.DateTime(timezone=True), nullable=True))
    op.add_column('storefront_configs', sa.Column(
        'scheduled_publish_at', sa.DateTime(timezone=True), nullable=True))
    op.add_column('storefront_configs', sa.Column('published_by', postgresql.UUID(
    ...
    op.create_table(
        ...
        sa.Column('storefront_config_id', postgresql.UUID(as_uuid=True), sa.ForeignKey(
            'storefront_configs.id', ondelete='CASCADE'), nullable=False),
    ...
    op.create_index('idx_draft_storefront_config',
                    'storefront_drafts', ['storefront_config_id'])
    ...
    op.create_index('idx_storefront_version_created', 'storefront_versions', [
                    'storefront_config_id', 'created_at'])
    ...
    op.create_table(
        ...
        sa.Column('role', sa.Enum('viewer', 'editor', 'publisher', 'admin',
                  name='storefrontrole'), nullable=False, server_default='viewer'),
    ...
    op.create_index('idx_storefront_permission_user',
                    'storefront_permissions', ['user_id'])
    op.create_index('idx_storefront_permission_tenant',
                    'storefront_permissions', ['tenant_id'])
    ...
    op.create_table(
        ...
        sa.Column('asset_type', sa.Enum('image', 'video', 'document',
                  'audio', 'other', name='assettype'), nullable=False),
        ...
        sa.Column('parent_asset_id', postgresql.UUID(as_uuid=True),
                  sa.ForeignKey('storefront_assets.id'), nullable=True),
    ...
    op.create_index('idx_banner_active_tenant',
                    'storefront_banners', ['tenant_id', 'is_active'])
    ...
    op.create_table(
        ...
        sa.Column('is_active', sa.Boolean,
                  nullable=False, server_default='true'),
        sa.Column('asset_id', postgresql.UUID(as_uuid=True),
                  sa.ForeignKey('storefront_assets.id'), nullable=False),
    ...
    op.create_index('idx_component_tenant',
                    'storefront_components', ['tenant_id'])
    ...
    op.create_table(
        ...
        sa.Column('parent_template_id', postgresql.UUID(as_uuid=True),
                  sa.ForeignKey('storefront_page_templates.id'), nullable=True),
        sa.Column('inheritance_rules', postgresql.JSONB, nullable=False,
                  server_default='{"override": [], "extend": []}'),
    ...
    op.create_index('idx_template_tenant',
                    'storefront_page_templates', ['tenant_id'])
    op.create_index('idx_template_type', 'storefront_page_templates', [
                    'tenant_id', 'page_type'])
    op.create_index('idx_template_parent',
                    'storefront_page_templates', ['parent_template_id'])
