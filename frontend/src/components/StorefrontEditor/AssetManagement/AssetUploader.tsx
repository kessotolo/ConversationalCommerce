  ...
  const handleUpload = async () => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        formData.append('file', file); // file is safe to use now as we've added a null check
        formData.append('title', titles[fileName] || fileName);
        formData.append('asset_type', getAssetType(file)); // file is safe to use now

        // Add alt text only for images
        if (getAssetType(file) === 'image' && altTexts[fileName]) { // file is safe to use now
