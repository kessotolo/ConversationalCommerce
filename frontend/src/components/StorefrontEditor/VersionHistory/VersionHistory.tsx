...
const VersionHistory: React.FC<VersionHistoryProps> = ({ tenantId }) => {
  ...
  const filteredVersions = versions.filter((version) => {
    if (!version.created_at) return true; // Skip filter if no date
    
    const versionDate = new Date(version.created_at);
    if (isNaN(versionDate.getTime())) return true; // Skip filter if invalid date
    ...
    if (searchQuery && typeof searchQuery === 'string' && searchQuery.trim() !== '') {
      ...
      const matchesChangeSummary = version.change_summary && typeof version.change_summary === 'string' ? 
        version.change_summary.toLowerCase().includes(query) : false;
      const matchesChangeDescription = version.change_description && typeof version.change_description === 'string' ? 
        version.change_description.toLowerCase().includes(query) : false;
      const matchesTags = Array.isArray(version.tags) && version.tags.some((tag) => 
