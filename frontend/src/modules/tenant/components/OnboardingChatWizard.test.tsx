...
describe('OnboardingChatWizard', () => {
  ...
  it('handles file upload error', async () => {
    ...
    expect(await screen.findByTestId('onboarding-error')).toHaveTextContent(
  ...
  it('logs events for key actions', async () => {
    ...
    await waitFor(() => {
      ...
      const eventLoggerMock = require('../api/onboardingApi');
      expect(jest.mocked(eventLoggerMock.onboardingApi.uploadKYCFile)).toBeDefined();
    });
  });
  ...
  it('completes onboarding flow', async () => {
    ...
    await waitFor(() => {
      const onboardingComplete = screen.queryByText((content: string, element: Element | null) =>
        /onboarding complete/i.test(content)
      );
      expect(onboardingComplete).not.toBeNull();
      // eslint-disable-next-line @typescript-eslint/ban-ts-comment
      // @ts-expect-error TypeScript false positive: onboardingComplete is checked for null above.
      if (onboardingComplete) {
        expect(onboardingComplete).toBeInTheDocument();
      }
    });
  });
});
