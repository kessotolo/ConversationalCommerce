...
const UNUSED_IMPORTS_REPLACEMENTS: Replacement[] = [
  // Remove unused imports
  {
    from: /import\s+{[^}]*?Dialog[^}]*?}\s+from\s+['"].*?['"];?\n/g,
    to: ''
  },
  {
...
function fixIssuesInFile(filePath: string): void {
  ...
  UNUSED_IMPORTS_REPLACEMENTS.forEach(replacement => {
    ...
    if (typeof replacement.to === 'function') {
      newContent = fileContent.replace(replacement.from, (match: string, ...args: any[]) => {
        return (replacement.to as Function)(match, args[0], fileContent);
      });
    } else {
