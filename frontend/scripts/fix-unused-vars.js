...
const fixUnusedVars = (report) => {
  ...
  // Count total issues
  const totalIssues = report.reduce((sum, file) => 
    sum + file.messages.filter(msg => msg.ruleId === LINT_RULE).length, 0);
  
  console.log(`Attempting to fix ${totalIssues} issues in ${report.length} files...`);
  
  try {
    execSync(
      `npx eslint --rule '${LINT_RULE}: error' --fix "${SRC_DIR}/**/*.{ts,tsx}"`,
      { encoding: 'utf8' }
    );
    ...
    const remainingIssues = newReport.reduce((sum, file) => 
      sum + file.messages.filter(msg => msg.ruleId === LINT_RULE).length, 0);
