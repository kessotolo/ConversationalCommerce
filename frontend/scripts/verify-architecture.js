...
try {
  const results = execSync(`grep -r "from ['\\\"].*\/types\/" --include="*.ts" --include="*.tsx" ${SRC_DIR} | grep -v "test-eslint-rules.tsx" | grep -v ".bak"`, { encoding: 'utf-8' });
...
try {
  if (fs.existsSync(MODULES_DIR)) {
    allModules = fs.readdirSync(MODULES_DIR)
...
if (boundaryViolations.length > 0) {
  ...
  boundaryViolations.forEach(violation => {
    console.error(`   - ${violation.module} imports from ${violation.importedModule} in ${violation.file}`);
