...
const fixComponentProps = (filePath) => {
  ...
  const existingIconImport = updatedContent.match(/import\s*{([^}]*)}\s*from\s*['"]lucide-react['"]/);
  if (existingIconImport) {
    ...
    if (updatedContent.includes('<User') && !iconsList.includes('User')) 
      missingIcons.push('User');
