...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column('is_tenant_scoped', sa.Boolean(),
                  nullable=False, server_default='false'),
    ...
    # Note: permissionscope enum is created by SQLAlchemy when the table is created
    ...
    op.create_table(
        ...
        sa.Column('scope', sa.Enum('global', 'tenant', 'self',
                  name='permissionscope'), nullable=False, server_default='tenant'),
        ...
        sa.Column('updated_at', sa.DateTime(
            timezone=True), onupdate=sa.func.now()),
        sa.UniqueConstraint('resource', 'action', 'scope',
    ...
    op.create_table(
        ...
        sa.Column('id', postgresql.UUID(as_uuid=True),
                  primary_key=True, default=uuid.uuid4),
        sa.Column('parent_role_id', postgresql.UUID(as_uuid=True), sa.ForeignKey(
            'admin_roles.id', ondelete='CASCADE'), nullable=False),
        sa.Column('child_role_id', postgresql.UUID(as_uuid=True), sa.ForeignKey(
            'admin_roles.id', ondelete='CASCADE'), nullable=False),
        sa.Column('created_at', sa.DateTime(timezone=True),
                  server_default=sa.func.now()),
        sa.UniqueConstraint('parent_role_id', 'child_role_id',
    ...
    op.create_table(
        ...
        sa.Column('permission_id', postgresql.UUID(as_uuid=True), sa.ForeignKey(
            'admin_permissions.id', ondelete='CASCADE'), nullable=False),
        sa.Column('condition', sa.Text()),
        sa.Column('created_at', sa.DateTime(timezone=True),
                  server_default=sa.func.now()),
        sa.Column('updated_at', sa.DateTime(
    ...
    op.create_table(
        ...
        sa.Column('is_super_admin', sa.Boolean(),
                  nullable=False, server_default='false'),
        sa.Column('require_2fa', sa.Boolean(),
                  nullable=False, server_default='false'),
        sa.Column('allowed_ip_ranges', postgresql.JSONB(
            astext_type=sa.Text())),
    ...
    op.create_table(
        ...
        sa.Column('id', postgresql.UUID(as_uuid=True),
                  primary_key=True, default=uuid.uuid4),
        sa.Column('admin_user_id', postgresql.UUID(as_uuid=True), sa.ForeignKey(
            'admin_users.id', ondelete='CASCADE'), nullable=False),
        sa.Column('role_id', postgresql.UUID(as_uuid=True), sa.ForeignKey(
            'admin_roles.id', ondelete='CASCADE'), nullable=False),
        ...
        sa.UniqueConstraint('admin_user_id', 'role_id',
                            'tenant_id', name='uq_admin_user_role_tenant')
    )
    ...
    op.create_index('ix_admin_permissions_action',
                    'admin_permissions', ['action'])
    op.create_index('ix_admin_users_user_id', 'admin_users', ['user_id'])
    op.create_index('ix_admin_user_roles_admin_user_id',
                    'admin_user_roles', ['admin_user_id'])
    op.create_index('ix_admin_user_roles_role_id',
                    'admin_user_roles', ['role_id'])
    op.create_index('ix_admin_user_roles_tenant_id',
                    'admin_user_roles', ['tenant_id'])
